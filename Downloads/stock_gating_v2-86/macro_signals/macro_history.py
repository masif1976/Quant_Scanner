...
are replayed exactly:
    VIX Level, VIX Term Structure, Credit Spreads, VIX Momentum, Mega-Cap
    Rotation, Sector Breadth  (6 of 7, actually)
...
def _vix_momentum_series(vix: pd.Series) -> pd.Series:
    """VIX 20-day rate of change (volatility velocity). Falling/stable VIX
    momentum -> 100 (calm), surging VIX momentum -> 0 (fear accelerating)."""
...
def _breadth_series(sector_panel: pd.DataFrame) -> pd.Series:
    """
    Daily Sector Breadth: of the 11 SPDR sector ETFs, what fraction trade
    strictly above their 50-day SMA, mapped linearly to 0-100.

    Matches the live-engine sector_breadth() formula exactly so today's
    reading on Page 1 and the daily history series on the chart use the
    same definition — they cannot drift apart.
    """
    if sector_panel.empty:
        return pd.Series(dtype=float)
    sma50 = sector_panel.rolling(50, min_periods=25).mean()
    above = (sector_panel > sma50)
    # fraction of sectors above their 50-SMA on each day -> 0-100 score
    pct = above.sum(axis=1) / above.count(axis=1) * 100
    return pd.Series(_clip(pct.fillna(50)), index=sector_panel.index)
...
def regime_timeseries(watchlist: list | None = None) -> dict:
    ...
    try:
        ...
        # sector ETF panel for breadth (replaces the old watchlist-based panel)
        from macro_signals.signals_a import SECTOR_ETFS
        sector_data = du.get_bulk_history(SECTOR_ETFS, days=620)
        sector_closes = {}
        for t, df in sector_data.items():
            if not df.empty and "Close" in df:
                ...
                if len(s) > 60:
                    sector_closes[t] = s
        sector_panel = pd.DataFrame(sector_closes) if sector_closes else pd.DataFrame()
        ...
        series = {
            ...
            "VIX Momentum": _vix_momentum_series(vix),
            "Mega-Cap Rotation":  _megacap_series(mags, spy),
        }
        if not sector_panel.empty:
            series["Sector Breadth"] = _breadth_series(sector_panel)
        ...
        # Canonical composite — same formula used by run_macro_gate.compute_composite
        # for the "today" reading on Page 1, ensuring history and current value
        # never drift apart.
        import run_macro_gate as _rmg
        composite = score_df.apply(
            lambda row: _rmg.compute_composite(row.to_dict()), axis=1
        ).round(1)
