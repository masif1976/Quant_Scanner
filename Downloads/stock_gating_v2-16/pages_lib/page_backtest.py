...
def render():
    ...
    if macro is None:
        st.info("Run the analysis from the sidebar — the Macro Gate must clear first.")
        return
        ...
        st.markdown(
            ...
            <div class='panel' style='text-align:center;padding:46px;
              ...
              <div class='tiny' style='margin-top:14px'>
                ...
                <b>{macro['composite_score']:.0f}</b> — DEFENSIVE regime.<br>
    ...
    st.plotly_chart(_backtest_chart(bt), use_container_width=True,
                    config={"displayModeBar": False})

    # directional-bias legend
    legend = [("🟢 STRONG LONG", theme.GREEN), ("🟢 LEAN LONG", "#7fd98a"),
              ("🟡 HOLD / CASH", theme.YELLOW), ("🟠 WATCH SHORT", theme.ORANGE),
              ("🔴 LEAN SHORT", "#ff7a6c"), ("🔴 STRONG SHORT", theme.RED)]
    chips = "".join(
        f"<span style='display:inline-block;margin:3px 6px 3px 0;"
        f"padding:3px 11px;border-radius:6px;background:{c}22;color:{c};"
    ...
    for label, b in buckets.items():
        perf_rows.append({
            "Status": label,
            "Days in Status": b["days"],
            f"Avg {bt['fwd_window']}d Fwd Return": (
                f"{b['avg_fwd_ret']:+.2f}%" if b["avg_fwd_ret"] is not None
                else "—"),
    ...
    def _style_status(val):
        s = str(val)
        if "STRONG LONG" in s:  return f"color:{theme.GREEN};font-weight:700"
        if "LEAN LONG" in s:    return "color:#7fd98a;font-weight:700"
        if "HOLD" in s:         return f"color:{theme.YELLOW};font-weight:700"
        if "WATCH SHORT" in s:  return f"color:{theme.ORANGE};font-weight:700"
        if "LEAN SHORT" in s:   return "color:#ff7a6c;font-weight:700"
        if "STRONG SHORT" in s: return f"color:{theme.RED};font-weight:700"
        return ""
    ...
    with bcol:
        fr = f"{best['fwd_ret']:+.2f}%" if best["fwd_ret"] is not None else "—"
        st.markdown(
            f"""
            <div class='panel' style='border-color:{theme.GREEN}55'>
              ...
              <div class='big-score' style='font-size:2.4rem;
                   color:{theme.GREEN};margin:8px 0'>{best['score']:.0f}</div>
              <div class='tiny' style='line-height:1.9'>
                Date &nbsp;<b style='color:{theme.TEXT}'>{best['date']}</b><br>
                Status &nbsp;<b>{best['status']}</b><br>
                Price &nbsp;<b style='color:{theme.TEXT}'>${best['price']:,.2f}</b><br>
                {bt['fwd_window']}d forward return &nbsp;
                <b style='color:{theme.GREEN}'>{fr}</b>
              </div>
            </div>
            """, unsafe_allow_html=True)

    with wcol:
        fr = f"{worst['fwd_ret']:+.2f}%" if worst["fwd_ret"] is not None else "—"
        st.markdown(
            f"""
            <div class='panel' style='border-color:{theme.RED}55'>
              <div class='kicker'>⚠️ Worst Signal — Lowest Score</div>
              <div class='big-score' style='font-size:2.4rem;
                   color:{theme.RED};margin:8px 0'>{worst['score']:.0f}</div>
              <div class='tiny' style='line-height:1.9'>
                Date &nbsp;<b style='color:{theme.TEXT}'>{worst['date']}</b><br>
                Status &nbsp;<b>{worst['status']}</b><br>
                Price &nbsp;<b style='color:{theme.TEXT}'>${worst['price']:,.2f}</b><br>
                {bt['fwd_window']}d forward return &nbsp;
                <b style='color:{theme.MUTED}'>{fr}</b>
              </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        historically. {bt['n_days']} trading days audited. Run: {datetime.fromisoformat(bt['timestamp']).strftime('%Y-%m-%d %H:%M')}
        </div>
        """, unsafe_allow_html=True)
