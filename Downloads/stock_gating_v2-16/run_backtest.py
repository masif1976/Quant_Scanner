...
for a single ticker, using whichever Strategy Engine is active.
...
replayed day by day. The backtest reconstructs the four price/volume factors
(Momentum, Volume Surge, Relative Strength, Range Proximity), re-weighted to
sum to 1.0, for whichever strategy is active. This is an honest audit of the
price/volume-driven component, which is what moves the live score day to day.
"""
...
# Historical backtest: 6-factor average matching the live scanner. Four
# price/volume factors are replayed from OHLCV; Short Interest and Options Flow
# have no free historical series, so each is pinned to a neutral 50 every day.
# (P/E and Social Sentiment are decoupled from the score entirely.)
NEUTRAL_FACTOR = 50.0
BT_REPLAYED = ["Momentum", "Volume Surge", "Relative Strength", "Range Proximity"]
BT_NEUTRAL_COUNT = 2  # Short Interest + Options Flow
BT_DIVISOR = 6        # 4 replayed + 2 neutral
...
def run(ticker: str, strategy: str = TREND) -> dict:
    ...
    # 7-factor average: 4 replayed factors + 3 neutral (50) factors, ÷ 7.
    # Short Interest, Options Flow, and Social Sentiment have no free historical
    # series, so each contributes a hardcoded neutral 50 for every day.
    composite = (
        (factor_df[BT_REPLAYED].sum(axis=1)
         + NEUTRAL_FACTOR * BT_NEUTRAL_COUNT) / BT_DIVISOR
    ).round(1)

    display = pd.DataFrame({"Close": close, "Score": composite}).dropna()
    if len(display) > 252:
    ...
    return {
        ...
        "weights": {"replayed_factors": BT_REPLAYED,
                    "neutral_factors": ["Short Interest", "Options Flow"],
                    "neutral_value": NEUTRAL_FACTOR, "divisor": BT_DIVISOR},
        "n_days": len(display),
    }
