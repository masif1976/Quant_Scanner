...
def _gauge_title(name: str, stale: bool = False) -> str:
    ...
    return (f"<b>{layman}</b>{stale_tag}<br>"
            f"<span style='font-size:12px; color:#9aa6bd'>{technical}</span>")
...
def _gauge(score, name=None, height=185, accent=None, stale=False):
    ...
    band_grn = "rgba(34,224,138,0.14)"
    gauge_bg = "rgba(255,255,255,0.02)"   # faint inner fill for arc contrast
    indicator_kwargs = dict(
        ...
        gauge={
            ...
            "steps": [
                {"range": [0, 40],  "color": band_red},
                {"range": [40, 70], "color": band_yel},
                {"range": [70, 100], "color": band_grn},
            ],
    ...
    if name is not None:
        indicator_kwargs["title"] = {
            ...
            "font": {"color": txt, "family": "Sora", "size": 15},
        }
...
def _regime_chart(history: dict, benchmark: str = "SPY"):
    ...
    # Theme-aware colours so nothing merges into a light background. The
    # benchmark price line in particular MUST contrast with the page (it was
    # near-white before, which vanished in light mode).
    is_light = theme.get_mode() == "light"
    axis_txt = "#3a424f" if is_light else "#e0e0e0"
    price_line = "#3a4252" if is_light else "#e0e0e0"   # dark slate on light
    macro_line = "#5a6fb0" if is_light else "#9bb1d4"   # deeper indigo on light
    hover_bg = "rgba(255,255,255,0.96)" if is_light else "rgba(15,18,26,0.95)"
    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color=macro_line, width=1.8),
            name="Macro Score",
    ...
    fig.update_yaxes(
        ...
        showgrid=True, gridcolor=theme.BORDER, color=axis_txt,
        row=1, col=1,
    )
    fig.update_yaxes(
    ...
    fig.update_xaxes(
        showgrid=False, color=axis_txt, showticklabels=False, row=1, col=1,
    )
    fig.update_xaxes(
        showgrid=False, color=axis_txt, row=2, col=1,
    )
    ...
    fig.update_layout(
        ...
        hoverlabel=dict(
            ...
            font=dict(family="JetBrains Mono", size=11, color=axis_txt),
        ),
        showlegend=True,
        legend=dict(orientation="h", y=1.08, x=0,
                    font=dict(size=10, color=axis_txt)),
        ...
        annotations=list(fig.layout.annotations or ()) + [
            dict(x=0.02, y=0.985, xref="paper", yref="paper",
                 ...
                 showarrow=False,
                 font=dict(family="JetBrains Mono", size=10, color=axis_txt)),
        ],
    )
...
def render():
    ...
    # ── 1. (Live Watchlist Prices widget moved to Page 2 — Scanner — so
    #        Page 1 stays focused purely on macro analysis.) ──
    ...
    with c_score:
        st.markdown(
            ...
            <div class='panel' style='text-align:center;border-color:{color}55;
              ...
              <div class='kicker' style='color:{theme.HERO_MUTED}'>Master Composite Macro Score</div>
              ...
              <div class='regime-pill' style='background:{color}22;color:{color}'>
                {regime}</div>
              <div class='tiny' style='margin-top:12px;color:{theme.HERO_MUTED}'>
