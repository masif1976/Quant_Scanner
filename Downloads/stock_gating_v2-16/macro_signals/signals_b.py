"""
Macro signals 4-7: Credit Spreads, Put/Call Sentiment, Factor Crowding, Sector Rotation.
"""
...
# ── 5. Put/Call Sentiment ─────────────────────────────────────────────────────
def put_call_sentiment() -> dict:
    name = "Put/Call Sentiment"
    try:
        closes = du.get_close_series("^VIX", days=60)
        if len(closes) < 22:
        ...
        return {
            "name": name, "score": round(score, 1), "status": "ok",
            "vix_roc_20d": round(roc, 1),
            "detail": f"VIX 20d ROC {roc:+.1f}% — {mood}",
        }
    except Exception as e:
        return _err(name, str(e))
