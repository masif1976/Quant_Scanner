...
def calculate_tranche_action(macro_score: float, stock_score: float) -> dict:
    ...
    Tranche logic:
      ...
      Stock 80-100 + Macro 40-69  -> 🟢 TRANCHE 2 (MID LONG)
      Stock 65-79                 -> 🟢 TRANCHE 1 (PILOT LONG)
      Stock 50-64                 -> 🟡 HOLD CORE / CASH
      Stock 35-49                 -> 🔴 TRANCHE 1 (PROBE SHORT)
      Stock 20-34                 -> 🔴 TRANCHE 2 (LEAN SHORT)
      Stock 0-19                  -> 🔴 TRANCHE 3 (MAX SHORT)

    Overriding rule: if Macro Score < 40 (DEFENSIVE), every LONG action is
    forced to "❌ RISK OFF: COLD CASH".
    """
...
def _scan_one(ticker: str, df: pd.DataFrame, spy_close: pd.Series,
              strategy: str, with_options: bool = True) -> dict:
    """Compute all per-ticker factor results for the given strategy."""
    f_mom = factors.momentum(df, strategy)
    f_vol = factors.volume_surge(df, strategy)
    f_rs  = factors.relative_strength(df, spy_close, strategy)
    f_rp  = factors.range_proximity(df, strategy)
...
def _composite(raw_results: dict, watchlist: list, rankable_raw: dict) -> list:
    ...
    for t in watchlist:
        ...
        for fname in FACTOR_WEIGHTS:
            if fname in rankable_raw:
                factor_scores[fname] = rankable_raw[fname].get(t, 50.0)
            else:
                factor_scores[fname] = fr.get(fname, {}).get("score", 50.0)
        ...
        # Directional bias is purely score-driven (guides LONG/SHORT placement).
...
def run(watchlist: list[str], strategy: str = TREND) -> dict:
...
def scan_universe(strategy: str = TREND, top_n: int = 5) -> dict:
    """
    Run the factor engine across the S&P 500 sample for Page 1's
    "Broad Market Top 5". Skips slow short-interest / options calls for speed.
    """
