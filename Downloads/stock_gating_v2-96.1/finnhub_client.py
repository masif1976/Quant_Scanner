...
#   2. Streamlit secrets ([finnhub] api_key, or top-level FINNHUB_API_KEY)
#   3. The fallback literal below (local-dev convenience ONLY)
#
# ⚠ SECURITY: the literal below is a convenience fallback so the app runs
# out-of-the-box locally. Before pushing to a PUBLIC repo: (a) set
# FINNHUB_API_KEY in your environment or .streamlit/secrets.toml, (b) blank
# out or rotate the literal, and (c) ensure secrets files are .gitignored.
# Bots scrape public GitHub for API keys within hours.
_FINNHUB_API_KEY_FALLBACK = "d89a5ipr01qla01mnq30d89a5ipr01qla01mnq3g"


def _api_key() -> str:
    """Resolve the Finnhub API key, preferring secure sources.

    Order: environment variable → Streamlit secrets → hardcoded fallback.
    This lets you secure the key for a public repo (set the env var and
    blank the fallback) without changing any calling code, while still
    working out-of-the-box locally.
    """
    # 1. Environment variable (most portable, CI-friendly)
    env_key = os.environ.get("FINNHUB_API_KEY", "").strip()
    if env_key:
        return env_key
    # 2. Streamlit secrets, if running inside Streamlit with a secrets file
    try:
        import streamlit as st
        # Support both [finnhub] api_key = "..." and top-level key
        if hasattr(st, "secrets"):
            if "finnhub" in st.secrets and "api_key" in st.secrets["finnhub"]:
                k = str(st.secrets["finnhub"]["api_key"]).strip()
                if k:
                    return k
            if "FINNHUB_API_KEY" in st.secrets:
                k = str(st.secrets["FINNHUB_API_KEY"]).strip()
                if k:
                    return k
    except Exception:
        pass  # not in Streamlit, or no secrets file — fall through
    # 3. Hardcoded fallback (local-dev convenience)
    return _FINNHUB_API_KEY_FALLBACK
