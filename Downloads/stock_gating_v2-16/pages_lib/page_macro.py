...
def _regime_chart(history: dict, benchmark: str = "SPY"):
    ...
    fill_opacity = {"FULL DEPLOY": 0.16, "REDUCED": 0.14, "DEFENSIVE": 0.16}
...
def _regime_legend():
    ...
    items = [("FULL DEPLOY", "#22e08a"), ("REDUCED", "#f5c344"),
             ("DEFENSIVE", "#ff5d6c")]
    chips = "".join(
...
def _render_price_banner(watchlist):
    ...
    st.markdown(
        "<div class='tiny'>Quotes via yfinance fast_info — current price &amp; "
        "daily change only (no historical download).</div>",
        unsafe_allow_html=True)
...
_METRIC_DEFS = [
    ...
    ("Credit Spreads", "The Bond Market Warning",
     ...
     "+2 Z-score", "-2 Z-score"),
    ("Put/Call Sentiment", "The Crowd's Bets",
     "Measures the 20-day rate of change in protective options demand.",
     "A +50% surge in volatility / puts over 20 days (Retail fear).",
     "A -30% drop in volatility over 20 days (Retail confidence).",
     "+50% ROC", "-30% ROC"),
