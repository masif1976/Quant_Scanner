...
def _render_risk_reward_scatter(headlines: list[dict]):
    ...
    # Quadrant labels (anchored in each corner of the chart). Full opacity +
    # larger size + a readable slate for the neutral quadrants so they don't
    # wash out against the light chart background.
    label_neutral = "#5b6675"   # readable slate (was theme.MUTED, too faint)
    annotations = [
        dict(x=88, y=y_max * 0.96, text="🎯 Best", showarrow=False,
              font=dict(color=theme.GREEN, size=13, family="JetBrains Mono")),
        dict(x=30, y=y_max * 0.96, text="⚠ Chasing yield", showarrow=False,
              font=dict(color=theme.RED, size=13, family="JetBrains Mono")),
        dict(x=88, y=15, text="🛡 Safe but thin", showarrow=False,
              font=dict(color=label_neutral, size=13, family="JetBrains Mono")),
