...
if "app_theme" not in st.session_state:
    # User-controlled light/dark theme. Defaults to light (more eye-friendly
    # for extended use); the toggle in sidebar Settings switches to dark.
    st.session_state.app_theme = "light"


# Apply the active theme BEFORE injecting CSS so the stylesheet reflects it.
theme.set_mode(st.session_state.app_theme)
...
def render_sidebar():
    with st.sidebar:
        ...
        # ── Navigation ──
        page_options = ["MarketSense", "Scanner & Status", "Fundamental Insights",
                        "Backtest & Audit", "Strategy Backtest",
                        "Trade Journal", "Positions", "Options Wheel"]
        ...
        # Stable widget key so Streamlit persists the selection across
        # reruns instead of resetting to default= each time. Without a key,
        # an unrelated rerun (any other widget interaction) could momentarily
        # reset the multiselect, which is the root of the "deleted ticker
        # reappears" symptom. The key is scoped per active watchlist so
        # switching lists shows the right contents.
        wl_widget_key = f"wl_multiselect_{active_name}"
        # Options: current tickers PLUS the default set, so the user can
        # re-add a default they previously removed. The selection itself
        # (what's saved) is independent of this option pool.
        wl_options = sorted(set(st.session_state.watchlist) |
                            set(du.DEFAULT_WATCHLIST))
        # Seed the widget's session_state once, then let the key own it.
        if wl_widget_key not in st.session_state:
            st.session_state[wl_widget_key] = list(st.session_state.watchlist)
        ...
        # Detect a real change vs the persisted list and write through.
        # Compare as sets so pure reordering doesn't trigger a needless save,
        # but content changes (add/remove) always persist.
        if set(wl) != set(st.session_state.watchlist):
            ...
            if st.button("＋ Add", width="stretch"):
                ...
                for raw in new.replace(",", " ").split():
                    ...
                    if t and t not in st.session_state.watchlist:
                        ...
                        changed = True
                if changed:
                    # Clear the keyed multiselect so it re-seeds from the
                    # updated watchlist on rerun (keyed widgets ignore
                    # default= once their session_state exists).
                    st.session_state.pop(f"wl_multiselect_{active_name}", None)
                    st.rerun()
        with c2:
        ...
        with st.expander("⚙️ Settings"):
            new_theme = st.radio(
                "Theme",
                options=["dark", "light"],
                index=["dark", "light"].index(st.session_state.app_theme),
                format_func=lambda s: {"dark": "🌙 Dark",
                                        "light": "☀️ Light"}[s],
                key="app_theme_radio",
                help="Switch the entire app between dark and light. Affects "
                     "the shell, panels, and all charts.",
            )
...
elif st.session_state.page == "Fundamental Insights":
    from pages_lib import page_fundamentals
    page_fundamentals.render()
