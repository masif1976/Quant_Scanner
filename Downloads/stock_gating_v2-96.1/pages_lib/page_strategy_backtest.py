...
- **Watchlist survivorship bias.** The backtest runs over *today's*
  watchlist. Names that delisted, collapsed, or you removed after a bad
  run are absent — so historical results skew optimistic (the losers
  that would have dragged returns down aren't in the sample). Price/
  volume signals are partly insulated (they only use each surviving
  name's own history), but be aware the *set* of tickers is itself a
  winner's bracket.
- **Point-in-time fundamentals.** Any fundamental-weighted element uses
  *latest* figures applied to *historical* dates, when those figures
  weren't yet public. This is a form of look-ahead that flatters
  fundamental-driven results. The 4 replayed factors here are price/
  volume only (immune), but if you ever weight fundamentals into a
  historical backtest, treat long-horizon results with suspicion. A true
  fix needs a point-in-time fundamentals database — out of scope for a
  free-tier tool, so the honest remedy is this disclosure.
- **Entry fill timing (look-ahead corrected).** Signals are computed from
  a day's *closing* data, so the earliest tradeable fill is the **next
  bar**. Entries therefore fill at the next bar's close after each
  signal — never the signal bar's own close. This avoids same-bar
  look-ahead (buying at a price you couldn't have known when the signal
  fired) and makes the reported returns honest about execution.
""")
...
def render():
    ...
    # ── backtest configuration (cost realism + sizing) ──
    # Surfaced per audit M-3: the transaction-cost assumption materially
    # affects results and users should be able to stress it. Mid-cap and
    # options spreads are often wider than the 7.5 bps default.
    with st.expander("⚙️ Backtest assumptions (cost & sizing)", expanded=False):
        st.markdown(
            f"<div style='font-family:JetBrains Mono;font-size:0.78rem;"
            f"color:{theme.MUTED};line-height:1.5;margin-bottom:8px'>"
            f"These assumptions shape the net results. The cost default "
            f"(7.5 bps/side) reflects tight large-cap spreads in the "
            f"zero-commission era; mid-caps, thin names, and options "
            f"routinely cost more. Raise it to stress-test how quickly a "
            f"thin edge gets eaten by real frictions.</div>",
            unsafe_allow_html=True)
        cfg_c1, cfg_c2 = st.columns(2)
        with cfg_c1:
            cost_bps_cfg = st.slider(
                "Transaction cost (bps per side)",
                min_value=0.0, max_value=50.0,
                value=float(pb.DEFAULT_COST_BPS_PER_SIDE), step=0.5,
                key="bt_cost_bps",
                help="Basis points charged on BOTH entry and exit. "
                     "Round-trip = 2× this. 0 = frictionless (unrealistic); "
                     "7.5 = tight large-cap; 20-40 = mid-cap / options / "
                     "wide spreads.")
        with cfg_c2:
            target_d_cfg = st.slider(
                "Position size per trade ($)",
                min_value=1000, max_value=50000,
                value=int(pb.DEFAULT_TARGET_DOLLARS), step=1000,
                key="bt_target_dollars",
                help="Equal-dollar exposure per signal. Affects share "
                     "counts and absolute P&L, not percentage returns.")
        st.markdown(
            f"<div style='font-family:JetBrains Mono;font-size:0.74rem;"
            f"color:{theme.MUTED};margin-top:4px'>"
            f"Round-trip cost: <b style='color:{theme.TEXT}'>"
            f"{cost_bps_cfg*2:.1f} bps</b> "
            f"({cost_bps_cfg*2/100:.3f}% per trade). Results below are "
            f"<b>net</b> of this. Entries fill at the <b>next bar's close</b> "
            f"after each signal (no same-bar look-ahead).</div>",
            unsafe_allow_html=True)
    ...
    if st.button("▶  Run Watchlist Backtest", type="primary",
        ...
        with st.spinner(f"Replaying {len(watchlist)} tickers over the last "
            ...
            bt = pb.run(watchlist, strategy,
                         ...
                         target_dollars=float(target_d_cfg),
                         cost_bps_per_side=float(cost_bps_cfg))
