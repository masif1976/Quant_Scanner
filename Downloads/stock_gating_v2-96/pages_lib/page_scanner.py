...
def render():
    ...
    st.markdown(
        ...
        <div class='panel' style='text-align:center;border-color:{regime_color}55;
          ...
          <div class='kicker' style='color:{theme.HERO_MUTED}'>Current Macro Regime</div>
...
def _render_earnings_surprises(sel_ticker: str):
    ...
    panel = (
        f"<div style=\"background:linear-gradient(160deg,#121724,#0d1320);"
        ...
        f"<div>"
        f"<span style=\"color:#9aa6bd;font-family:JetBrains Mono;"
            ...
            if em is None:
                ...
                if atr_val and atr_val > 0:
                    ...
                    atr_note = (f"Levels are multiples of ATR(14) = "
                                ...
                                f"your target</b>.")
                    # Horizon-mismatch nudge: ATR stop/target are sized for a
                    # multi-week swing (the backtest holds ~20 days). If the
                    # projection horizon is much shorter (e.g. 7d weekly), a
                    # 3-ATR target will show a tiny touch probability simply
                    # because 3 daily ranges rarely happen in a week — that's
                    # a timescale artifact, not a hopeless trade. Warn so the
                    # user doesn't misread it.
                    if int(effective_dte) <= 10:
                        atr_note += (
                            f"<br><b style='color:{theme.YELLOW}'>⚠ Timescale "
                            f"note:</b> these ATR levels are sized for a "
                            f"multi-week swing (the backtest holds ~20 days), "
                            f"but you're projecting over only "
                            f"{int(effective_dte)} days. A 3-ATR target "
                            f"naturally shows a tiny probability over a short "
                            f"window — that's a timescale mismatch, not a "
                            f"hopeless trade. <b>Set the Projection horizon to "
                            f"3-4 weeks</b> to match the holding period these "
                            f"levels were designed for.")
                else:
...
def _render_grade_card(sel_row: dict, sel_ticker: str):
    ...
    card_html = (
        f"<div style=\"background:linear-gradient(160deg,#121724,#0d1320);"
        f"border:1px solid {grade_color}55;"
        f"border-left:4px solid {grade_color};border-radius:10px;"
        ...
        f"border-top:1px solid #2a3550;line-height:1.5\">"
        f"{footer_text}"
        f"</div>"
        f"</div>"
    )
...
def _render_technical_chart(ticker: str):
    ...
    # Theme-aware "ink" colour for axis text and the ROC line — dark on a
    # light page, light on dark — so nothing merges into a white background.
    is_light = theme.get_mode() == "light"
    LIGHT = "#3a424f" if is_light else "#e0e0e0"
    ...
    layout_kwargs = dict(
        template="plotly_white" if is_light else "plotly_dark", barmode="overlay",
