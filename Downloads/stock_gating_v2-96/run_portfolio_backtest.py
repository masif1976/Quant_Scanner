...
def _simulate_trade(signal: dict, series: pd.DataFrame,
    ...
    signal_loc = series.index.get_loc(entry_date)
    # ── Next-bar entry (look-ahead fix) ──
    # The signal is computed from the SIGNAL bar's closing data (the score
    # crossed its threshold using that day's close). In live trading you
    # only observe that close after the session ends, so the earliest you
    # can actually transact is the NEXT bar. Filling at the signal bar's
    # own close would be same-bar look-ahead — buying at a price you could
    # not have known when the signal fired, which inflates results.
    #
    # We therefore fill at the next bar's close (signal_loc + 1). The
    # series only carries Close (not Open), so next-close is the honest,
    # tradeable choice: see today's close after hours → order fills at
    # tomorrow's close. The 20-day hold runs from this real entry bar.
    entry_loc = signal_loc + 1
    if entry_loc >= len(series):
        # Signal fired on the last available bar — no next bar to fill on.
        # Can't honestly simulate an entry, so skip (not counted in stats).
        return {**signal, "status": "skipped",
                "reason": "signal on final bar — no next-bar fill available"}

    exit_loc = entry_loc + HOLD_DAYS
    entry_price = float(series["Close"].iloc[entry_loc])
    entry_fill_date = series.index[entry_loc]
    ...
    if exit_loc >= len(series):
        ...
        return {
            **signal, "status": "open",
            ...
            "entry_fill_date": entry_fill_date.strftime("%Y-%m-%d"),
